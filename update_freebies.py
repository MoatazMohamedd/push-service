...
def main():
    ...
    for gp_game in gp_games:
        ...
        # If game already exists in Firestore, preserve it with updated API fields
        if gp_id in firestore_map:
            existing = firestore_map[gp_id]
            merged = {}
            for key, value in gp_game.items():
            ...
            # New game - fetch IGDB data
            print(f"🔎 Enriching new game: {gp_game['title']}")
